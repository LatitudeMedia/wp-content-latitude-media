...
if (!function_exists('ltm_styles')) {
    ...
    function ltm_styles()
    {
        ...
        wp_enqueue_style(array('ltm-header', 'ltm-footer'));
    }
...
{
    //
    $critical_css =  file_get_contents(get_template_directory() . '/dist/css/base.min.css');

    // Detect the news-with-hero-section or news-list-with-hero-section-block and loading assets as critical.
    if (
        has_block('acf/news-with-hero-section')
        || has_block('acf/news-list-with-hero-section-block')
    ) {
        $critical_css .= file_get_contents(get_template_directory() . '/dist/css/blocks/news-with-hero.min.css');
    }

    // Detect the categories-section-block and loading assets as critical.
    if (has_block('acf/categories-section-block') || is_category()) {
        $critical_css .= file_get_contents(get_template_directory() . '/dist/css/blocks/categories-section-block.min.css');
    }

    // Detect the landing podcast and loading assets as critical.
    if (has_block('acf/large-podcasts-section')) {
        $critical_css .= file_get_contents(get_template_directory() . '/dist/css/blocks/large-podcasts-section.min.css');
    }

    // Detect the landing podcast and loading assets as critical.
    if (has_block('acf/large-event-section')) {
        $critical_css .= file_get_contents(get_template_directory() . '/dist/css/blocks/featured-research-block.min.css');
    }

    // Detect the landing podcast and loading assets as critical.
    if (has_block('acf/research-banner-block')) {
        $critical_css .= file_get_contents(get_template_directory() . '/dist/css/blocks/research-banner-block.min.css');
    }

    // Detect the landing podcast and loading assets as critical.
    if (has_block('acf/content-with-background-block')) {
        $critical_css .= file_get_contents(get_template_directory() . '/dist/css/blocks/content-with-background-block.min.css');
    }

    // Detect the categories-section-block and loading assets as critical.
    if (is_front_page()) {
        $critical_css .= file_get_contents(get_template_directory() . '/dist/css/blocks/sidebar-editors-picks-section.min.css');
    } else {
        $critical_css .= file_get_contents(get_template_directory() . '/dist/css/post.min.css');
    }

    // Top podcasts template vs default single podcast.
    if (is_page_template('template-top-podcasts.php')) {
        $critical_css .= file_get_contents(get_template_directory() . '/dist/css/top-podcasts.min.css');
    ...
    if (is_singular(['resources'])) {
        $critical_css .= file_get_contents(get_template_directory() . '/dist/css/single-resource.min.css');
    }
